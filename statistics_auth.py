...
child = pexpect.spawn('ssh xxx@shell2015.icec.tf -p 22')
child.expect("ctf-4092@shell2015.icec.tf's password:")
child.sendline ('') # Password
child.expect('.* Have fun!')
child.sendline('nc vuln2015.icec.tf 9000')
child.expect(".*\nGimme .*")
out = re.search('\d+ .*\\n.*', child.after).group(0)
print(repr(out))
while('Gimme' in out):
